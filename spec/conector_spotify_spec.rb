...
require "#{File.dirname(__FILE__)}/../dominio/conector_spotify"
require 'dotenv/load'
...
describe ConectorSpotify do
  let(:logger) { FakeLogger.new }

  it 'deberia obtener un artista similar' do
    obtengo_token
    obtengo_id_de_artista('fake', 'Charly Garcia', 'unId')
    obtengo_artista_similar('fake', 'unId', 'Pedro Aznar')
    expect(described_class.new(logger).obtener_artista_similar(Artista.new('Charly Garcia')).nombre).to eq 'Pedro Aznar'
  end
end
